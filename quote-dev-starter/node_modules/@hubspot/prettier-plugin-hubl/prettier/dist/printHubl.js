...
const { builders: { group, indent, dedent, join, hardline, line, softline, align }, } = prettier_1.doc;
...
function printHubl(node) {
    ...
    switch (node.typename) {
        ...
        case "If":
            ...
            ifParts.push(openTag(node.whiteSpace.closingTag), " endif ", closeTag(node.whiteSpace.closingTag));
        ...
        case "Literal":
            ...
            if (typeof node.value === "string") {
                return prettier_1.util.makeString(node.value, '"');
            }
            return `${node.value}`;
        case "Comment":
            return node.value;
        ...
        case "FunCall":
            return [
                printHubl(node.name),
                "(",
                join(", ", node.args.children.map((arg) => {
        ...
        case "KeywordArgs":
            return [
                hardline,
                join([",", hardline], node.children.map((kw) => {
                    return group([kw.key.value, "=", printHubl(kw.value)]);
                })),
            ];
        case "Dict":
            const dictParts = ["{"];
            dictParts.push(indent(join(",", node.children.map((kw) => {
