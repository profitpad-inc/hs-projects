...
class Parser extends Obj {
    ...
    parseCall() {
        ...
        const callNode = new nodes.Caller(callTok.lineno, callTok.colno, callerArgs, macroCall, body);
    ...
    parseImport() {
        ...
        if (this.skipSymbol("as")) {
            ...
            node = new nodes.Import(importTok.lineno, importTok.colno, template, target, withContext);
        }
        else {
    ...
    parseFrom() {
        ...
        const fromNode = new nodes.FromImport(fromTok.lineno, fromTok.colno, template, names, withContext);
    ...
    parseRaw(tagName) {
        ...
        const rawBlockRegex = new RegExp("([\\s\\S]*?)({%-*)\\s*(" +
            tagName +
            "|" +
            endTagName +
            ")\\s*(?=-*%})(-*%})");
    ...
    parsePostfix(node) {
        ...
        while (tok) {
            if (tok.type === lexer.TOKEN_LEFT_PAREN) {
                ...
                node = new nodes.FunCall(tok.lineno, tok.colno, node, new nodes.NodeList(tok.lineno, tok.colno, [this.parseExpression()]));
            }
            else {
                break;
            }
    ...
    parseExpression() {
        ...
        if (this.skipSymbol("?")) {
            ...
            const condNode = node;
            node = new nodes.Ternary(node.lineno, node.colno);
            node.body = bodyNode;
            node.cond = condNode;
    ...
    parseOr() {
        ...
        while ((this.skip(lexer.TOKEN_PIPE) && this.tokens._extractString("|")) ||
            this.skipSymbol("or")) {
