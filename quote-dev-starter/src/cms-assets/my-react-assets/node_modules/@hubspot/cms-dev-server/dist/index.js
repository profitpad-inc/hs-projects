...
function trackUsage(action) {
  const accountConfig = trackingAccountId && getConfigAccountIfExists(trackingAccountId);
...
async function proxySpecificPage(request, response, portalId, hublet, contentId, webRoot, viteServer, projectName, projectComponentPath, jsRenderingGates, urlToProxy, fullOriginalUrl) {
  let account;
  let proxyEnv;
  const prettyPrint = isDebugUrlString3(urlToProxy);
  try {
    account = getConfigAccountIfExists2(portalId);
    if (!account) {
      throw new Error();
    }
...
async function renderLocalProjectModulePreview(moduleName, authedPortalId, authedEnv, authedHublet, webRoot, viteServer, jsRenderingGates, projectName, projectComponentPath, fullOriginalUrl, cookieValue, fieldOverrides = {}) {
  ...
  try {
    const authHeaders = {};
    const accountConfig = getConfigAccountIfExists3(authedPortalId);
...
var moduleFieldsJsonRouteHandler = async (request, response) => {
  try {
    ...
    const { webRoot, viteServer } = request.app.locals;
...
async function fetchContentMetadata(url3, portalId) {
  ...
  if (Array.isArray(portalIdHeader) || Array.isArray(contentId)) {
    ...
    const pageAccount = await getConfigAccountIfExists4(portalIdFromResponse);
    if (!pageAccount) {
      throw new Error(
...
function getShadowDevServerCheckHandler(devServerPort, themePath) {
  return async function shadowDevServerCheckHandler(request, response) {
    ...
    if (query) {
      ...
      let localProxyUrl;
      try {
        account = getConfigAccountIfExists5(portalId);
        if (!account) {
          throw new Error();
        }
      } catch (e) {
...
// src/createDevServer.ts
import {
  getConfigAccountIfExists as getConfigAccountIfExists6,
...
var isInModulesFolder = (filePath, projectRoot) => {
  const getEntryPathFromId = (id) => {
    return id.startsWith(projectRoot) ? id.replace(`${projectRoot}${path15.sep}`, "") : "";
  };
...
function getWebRootProjectComponentFolder(absoluteWebRoot, projectConfigPath) {
  ...
  try {
    const projectDir = path16.dirname(projectConfigPath);
    const configContent = fs7.readFileSync(projectConfigPath, "utf8");
    ...
    if (isNewStructure) {
      ...
      const hsmetaPath = path16.join(
        projectDir,
        srcDir,
        "theme",
        "theme-hsmeta.json"
      );
      if (fs7.existsSync(hsmetaPath)) {
        const hsmetaContent = fs7.readFileSync(hsmetaPath, "utf8");
...
async function createDevServer(webRoot, storybook, hubspotCliConfigPathOption, hubspotCliAccountNameOption, sslEnabledOption, fieldGenEnabledOption, themePreviewOptions, fieldBuilderEnabled) {
  ...
  if (hubspotCliConfigPathOption) {
    process.env.HUBSPOT_CONFIG_PATH = hubspotCliConfigPathOption;
  }
  ...
  if (isValid) {
    const account = hubspotCliAccountNameOption ? getConfigAccountIfExists6(hubspotCliAccountNameOption) : getConfigDefaultAccountIfExists();
    if (!account) {
      console.error(
        hubspotCliAccountNameOption ? `Couldn't find ${hubspotCliAccountNameOption} in your hubspot.config.yaml` : `No default account found. Run 'hs auth' first.`
      );
      process.exit(1);
    }
...
Local proxy functionality and module preview routes will not work without setting up authentication. You can work purely on modules locally at "/module/[module_name]", however the experience may be degraded due to missing account data.`
  ...
  if (fieldGenEnabledOption) {
    const normalizedAbsoluteWebRoot = path16.normalize(absoluteWebRoot);
