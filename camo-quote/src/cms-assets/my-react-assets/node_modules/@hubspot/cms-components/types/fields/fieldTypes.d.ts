...
export declare type BaseField = {
    ...
    cssVariable?: string;
    cssVariableState?: string;
};
export declare type BaseVisibility = {
    ...
    occurrence_options?: {
        count?: number;
        operator?: 'NOT_EQUAL' | 'EQUAL' | 'EMPTY' | 'NOT_EMPTY' | 'GREATER_THAN' | 'GREATER_THAN_OR_EQUAL' | 'LESS_THAN' | 'LESS_THAN_OR_EQUAL';
    };
    property?: string;
};
export declare type Visibility = BaseVisibility & {
    hidden_subfields?: {} | null;
    value_on_hide?: {};
};
export declare type AdvancedVisibility = {
    boolean_operator: 'AND' | 'OR';
    criteria: BaseVisibility[];
    children?: AdvancedVisibility[];
};
export declare type DisabledControlsRules = {
    boolean_operator?: 'AND' | 'OR';
    criteria?: BaseVisibility[];
    children?: DisabledControlsRules[];
    rules?: DisabledControlsRules;
};
export declare type DisabledControls = {
    message?: string;
    rules: DisabledControlsRules;
};
...
declare type BorderSide = 'top' | 'right' | 'bottom' | 'left' | 'all';
declare type BorderSubfieldName = 'width' | 'opacity' | 'style' | 'color';
export declare type BorderSubfieldPath = `${BorderSide}.${BorderSubfieldName}`;
export declare type BorderHiddenSubfields = Partial<Record<BorderSubfieldPath, boolean>>;
export declare type BorderFieldType = Omit<BaseField, 'visibility'> & {
    type: 'border';
    allowCustomBorderSides?: boolean;
    binding?: {
        context: 'global_styles';
        collection: ColorCollection;
    };
    visibility?: Omit<Visibility, 'hidden_subfields'> & {
        hidden_subfields?: BorderHiddenSubfields | null;
    };
    default?: {
        top?: BorderSideType;
...
declare type TextDecorationChoices = [[string, 'None'], [string, 'Underline']];
declare type CardShapeChoices = [
    [
        string,
        'Extra rounded'
    ],
...
declare type Presets = {
    ...
    case: CaseChoices;
    icon_shape: IconShapeChoices;
    filled: ShapeChoices;
    unfilled: ShapeChoices;
    text_decoration: TextDecorationChoices;
    card_shape: CardShapeChoices;
};
...
declare type BaseChoiceField = BaseField & SharedChoiceFieldTypes & {
    ...
    type: 'choice';
    choices: [any, string][];
    display?: 'radio' | 'checkbox' | 'select' | 'buttons' | 'pill';
...
export declare type ChoiceFieldType = BaseChoiceField | ChoiceFieldWithPresetProperty<'expand_icon'> | ChoiceFieldWithPresetProperty<'layout'> | ChoiceFieldWithPresetProperty<'icon_size'> | ChoiceFieldWithPresetProperty<'social_icon_size'> | ChoiceFieldWithPresetProperty<'icon_background_shape'> | ChoiceFieldWithPresetProperty<'social_icon_background_shape'> | ChoiceFieldWithPresetProperty<'case'> | ChoiceFieldWithPresetProperty<'icon_shape'> | ChoiceFieldWithPresetProperty<'filled'> | ChoiceFieldWithPresetProperty<'unfilled'> | ChoiceFieldWithPresetProperty<'text_decoration'> | ChoiceFieldWithPresetProperty<'card_shape'>;
declare type ColorCollection = 'theme_colors' | 'secondary_palette';
...
declare type IconSet = 'fontawesome-5' | 'fontawesome-5.0.10' | 'fontawesome-5.14.0' | 'fontawesome-6.4.2';
declare type IconValue = {
    name?: string;
    type?: 'SOLID' | 'REGULAR';
    unicode?: string;
};
declare type IconValueWithSet = IconValue & {
    set?: IconSet;
};
...
declare type ImageFeatures = 'replace' | 'crop';
export declare type ImageFieldType = BaseField & {
    type: 'image';
    resizable?: boolean;
    responsive?: boolean;
    showLoading?: boolean;
    enabledFeatures?: ImageFeatures[];
    default?: {
        src: string;
        alt?: string;
        width?: number;
...
export declare type VariantManagementFieldType = BaseField & {
    type: 'variantmanagement';
    variantDefinitionName: string;
    default?: undefined;
};
export declare type VariantSelectionFieldType = BaseField & {
...
export declare type GroupFieldType = BaseField & {
    ...
    display?: `drilldown` | `accordion` | `inline` | `tabs`;
    icon?: IconValueWithSet;
    preview?: string;
};
declare type CamelCaseField = AlignmentFieldType | AudioFieldType | BackgroundImageFieldType | BlogFieldType | BooleanFieldType | BorderFieldType | ChoiceFieldType | ColorFieldType | CrmObjectFieldType | CrmObjectPropertyFieldType | CtaFieldType | DateFieldType | DateTimeFieldType | EmailFieldType | EmbedFieldType | FileFieldType | FollowUpEmailFieldType | FontFieldType | FormFieldType | GradientFieldType | GroupFieldType | HtmlFieldType | HubDbRowFieldType | HubDbTableFieldType | HublFieldType | IconFieldType | ImageFieldType | LinkFieldType | LogoFieldType | MeetingFieldType | MenuFieldType | NumberFieldType | PageFieldType | PaymentFieldType | PodcastFieldType | RichTextFieldType | SfdcCamapaignFieldType | SimpleMenuFieldType | SpacingFieldType | SurveyFieldType | TagFieldType | TextAlignmentFieldType | TextFieldType | UrlFieldType | VariantManagementFieldType | VariantSelectionFieldType | VideoFieldType | WorkflowFieldType;
