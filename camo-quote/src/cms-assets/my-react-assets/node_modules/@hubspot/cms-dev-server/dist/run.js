...
function hasNewDirectoryStructure(platformVersion) {
  return parseFloat(platformVersion) >= 2025.2;
}
function readThemeHsmeta(projectDir, srcDir = "src") {
  const hsmetaPath = path.join(
    projectDir,
    srcDir,
    "theme",
    "theme-hsmeta.json"
  );
  if (!fs.existsSync(hsmetaPath)) {
    console.error(
      `Error: theme-hsmeta.json not found at expected path: ${hsmetaPath}. For platformVersion 2025.2 and higher, the project structure requires a theme-hsmeta.json file.`
    );
    process2.exit(1);
  }
  try {
    const hsmetaContent = fs.readFileSync(hsmetaPath, "utf8");
    const hsmeta = JSON.parse(hsmetaContent);
    if (!hsmeta.uid) {
      console.error(
        `Error: theme-hsmeta.json at ${hsmetaPath} is missing required 'uid' field.`
      );
      process2.exit(1);
    }
    return hsmeta;
    ...
    console.error(
      `Error: Failed to read or parse theme-hsmeta.json at ${hsmetaPath}`
    );
...
async function determineSrcAndDest(webRoot2, isUsingLocalWorkspace = false) {
  ...
  if (projectDir && projectConfig) {
    ...
    const platformVersion = projectConfig.platformVersion || "2025.1";
    const isNewStructure = hasNewDirectoryStructure(platformVersion);
    if (isNewStructure) {
      const srcDir = projectConfig.srcDir || "src";
      const hsmeta = readThemeHsmeta(projectDir, srcDir);
      const uid = hsmeta.uid;
      dest = `@projects/${projectConfig.name}/${uid}`;
    } else {
...
function uiAccountDescription(accountId, bold = true) {
  ...
  const message = `${account.name} [${HUBSPOT_ACCOUNT_TYPE_STRINGS[account.accountType]}] (${account.accountId})`;
...
function fullOriginalUrlFromRequest(request) {
...
// ../cms-js-internal/dist/types-BsFs0N9P.js
...
var serializeIslandDataForInlineScript = (tweakedIslandsArray, perServerRequestData, baseIndent = 0, prettyPrint) => {
...
var createIslandsGlobalJsForClientInDev = (roots, perServerRequestData, viteDevServer, indent, prettyPrint) => {
...
import { getConfigAccountIfExists as getConfigAccountIfExists2 } from "@hubspot/local-dev-lib/config";
...
function trackUsage(action) {
  const accountConfig = trackingAccountId && getConfigAccountIfExists2(trackingAccountId);
...
import { getConfigAccountIfExists as getConfigAccountIfExists3 } from "@hubspot/local-dev-lib/config";
...
var getPreviewUrl = (request, queryParams) => {
  const { authedPortalId, authedEnv, authedHublet, sslEnabled } = request.app.locals;
...
// src/proxyPage/proxyPage.ts
var getEntryPointBasePathFromProxyRequest = (request) => {
  if (!request) {
    return "";
  }
...
async function proxySpecificPage(request, response, portalId, hublet, contentId, webRoot2, viteServer, projectName, projectComponentPath, jsRenderingGates, urlToProxy, fullOriginalUrl) {
  let account;
  let proxyEnv;
  const prettyPrint = isDebugUrlString3(urlToProxy);
  try {
    account = getConfigAccountIfExists3(portalId);
    if (!account) {
      throw new Error();
    }
    proxyEnv = account == null ? void 0 : account.env;
  } catch (e) {
  ...
  try {
    const previewInfo = await fetchPreviewInfo_default(
      account.accountId,
      portalId,
      contentId
    );
    ...
    let pageHtml = (await makeProxyPreviewRequest(
      ...
      jsRenderingGates,
      request.get("COOKIE"),
      request.app.locals.localPreviewToken
    )).data;
...
import { getConfigAccountIfExists as getConfigAccountIfExists4 } from "@hubspot/local-dev-lib/config";
...
async function renderLocalProjectModulePreview(moduleName, authedPortalId, authedEnv, authedHublet, webRoot2, viteServer, jsRenderingGates, projectName, projectComponentPath, fullOriginalUrl, cookieValue, fieldOverrides = {}) {
  ...
  try {
    const authHeaders = {};
    const accountConfig = getConfigAccountIfExists4(authedPortalId);
...
var partialRouteHandler = async (request, response) => {
...
var linksToAllPartialRouteHandler = async (request, response) => {
  const { webRoot: webRoot2 } = request.app.locals;
...
var linksToAllModuleRouteHandler = async (request, response) => {
  const { webRoot: webRoot2 } = request.app.locals;
...
var moduleRouteHandler = async (request, response) => {
...
// src/routes/moduleFieldsJson.ts
var moduleFieldsJsonRouteHandler = async (request, response) => {
  try {
    sendTrackingRequest("fields-json-route");
    const { webRoot: webRoot2, viteServer } = request.app.locals;
...
import { getConfigAccountIfExists as getConfigAccountIfExists5 } from "@hubspot/local-dev-lib/config";
...
async function fetchContentMetadata(url3, portalId) {
  ...
  if (Array.isArray(portalIdHeader) || Array.isArray(contentId)) {
    ...
    const pageAccount = await getConfigAccountIfExists5(portalIdFromResponse);
    if (!pageAccount) {
      throw new Error(
...
async function proxyPageRouteHandler(request, response, next) {
...
function proxyPageResourceRedirect(req, res, next) {
  ...
  if (req.method === "GET") {
    ...
    const headersToPass = Object.fromEntries(headerEntries);
    let body = void 0;
    if (req.body) {
      if (isSendingJSON) {
...
async function fetchHubLModules(accountId, previewToken, limit = 250) {
  return (await hsCliHttp8.get(accountId, {
    url: `${DESIGN_MANAGER_API_PATH}/modules/local-preview?previewToken=${previewToken}&limit=${limit}`
  })).data;
}
async function fetchPreviewTemplates(accountId, previewToken, limit = 250) {
  return (await hsCliHttp8.get(accountId, {
    url: `${DESIGN_MANAGER_API_PATH}/templates/local-preview?previewToken=${previewToken}&limit=${limit}`
  })).data;
}
...
var legacyRouteHandler = async (request, response, next) => {
  const domainToProxy = getSubDomainFromValidLocalDomain(request.hostname);
  if (domainToProxy) {
    next();
    return;
  ...
  const { html: responseHtml } = await renderOnServer(
    /* @__PURE__ */ jsxs4(Fragment, { children: [
      storybookEnabled ? /* @__PURE__ */ jsx8("h2", { children: /* @__PURE__ */ jsx8("a", { href: `//storybook.${request.hostname}:${devServerPort}`, children: "Storybook" }) }) : null,
...
async function proxyPathPageRouteHandler(request, response, next) {
  ...
  let proxyPageUrl;
  try {
    proxyPageUrl = new URL(request.query.page);
  } catch (e) {
...
function proxyPathPageResourceRedirect(req, res, next) {
  ...
  fetch3(urlToProxy).then((response) => response.body.pipe(res)).catch((err) => console.log(err));
}
...
async function modulePreviewRouteHandler(request, response) {
  ...
  if (request.method === "POST" && request.body) {
    if (request.query[PREVIEWER_APP_PARAM] === "module" && request.body.module_data) {
      fieldOverrides = request.body.module_data;
    } else {
      fieldOverrides = request.body;
    }
    try {
...
var hublModuleRouteHandler = async (request, response) => {
  sendTrackingRequest("view-hubl-module-route");
  const { authedPortalId, localPreviewToken } = request.app.locals;
  ...
  const previewUrl = new URL(getPreviewUrl(request, params));
  try {
...
var templateRouteHandler = async (request, response) => {
  ...
  const environmentConfig = buildStaticEnvConfig5(
    authedPortalId,
    "na1",
    getConfigAccountEnvironment(authedPortalId),
    false,
    jsRenderingGates
  );
  ...
  const previewUrl = new URL(getPreviewUrl(request, params));
  const fullOriginalUrl = `${request.protocol}://${request.get("host")}${request.originalUrl}`;
  try {
...
`);
  ...
  const isProjectWithReact = consumerDeps.includes("@hubspot/cms-components") || process.env.TRANSLATE_ASSET;
...
var createDevServerRoutes = async (viteServer, appLocals, storybookPort) => {
  ...
  devServerApp.use((request, response, next) => {
    const { method, url: url3, hostname } = request;
...
import { getConfigAccountIfExists as getConfigAccountIfExists6 } from "@hubspot/local-dev-lib/config";
import cors from "cors";
function getShadowDevServerCheckHandler(devServerPort, themePath) {
  return async function shadowDevServerCheckHandler(request, response) {
    const { query } = request;
    if (query) {
      const {
        hostName,
      ...
      let localProxyUrl;
      try {
        account = getConfigAccountIfExists6(portalId);
        if (!account) {
          throw new Error();
        }
      } catch (e) {
...
// src/createDevServer.ts
import {
  getConfigAccountIfExists as getConfigAccountIfExists7,
...
var isInModulesFolder = (filePath, projectRoot) => {
  const getEntryPathFromId = (id) => {
    return id.startsWith(projectRoot) ? id.replace(`${projectRoot}${path16.sep}`, "") : "";
  };
...
function getWebRootProjectComponentFolder(absoluteWebRoot, projectConfigPath) {
  const pathSegments = absoluteWebRoot.replace(/\/$/, "").split("/");
  const themeFolderName = pathSegments[pathSegments.length - 1];
  if (!projectConfigPath) {
    return themeFolderName;
  }
  try {
    const projectDir = path17.dirname(projectConfigPath);
    const configContent = fs8.readFileSync(projectConfigPath, "utf8");
    const projectConfig = JSON.parse(configContent);
    const isNewStructure = hasNewDirectoryStructure(
      projectConfig.platformVersion
    );
    if (isNewStructure) {
      const srcDir = projectConfig.srcDir || "src";
      const hsmetaPath = path17.join(
        projectDir,
        srcDir,
        "theme",
        "theme-hsmeta.json"
      );
      if (fs8.existsSync(hsmetaPath)) {
        const hsmetaContent = fs8.readFileSync(hsmetaPath, "utf8");
        const hsmeta = JSON.parse(hsmetaContent);
        if (hsmeta.uid) {
          return hsmeta.uid;
        }
    ...
    console.warn(
      "Could not determine project component folder from config:",
      e.message
    );
  }
  return themeFolderName;
...
async function createDevServer(webRoot2, storybook2, hubspotCliConfigPathOption2, hubspotCliAccountNameOption2, sslEnabledOption2, fieldGenEnabledOption2, themePreviewOptions2, fieldBuilderEnabled2) {
  ...
  if (hubspotCliConfigPathOption2) {
    process.env.HUBSPOT_CONFIG_PATH = hubspotCliConfigPathOption2;
  }
  ...
  if (isValid) {
    const account = hubspotCliAccountNameOption2 ? getConfigAccountIfExists7(hubspotCliAccountNameOption2) : getConfigDefaultAccountIfExists();
    if (!account) {
      console.error(
        hubspotCliAccountNameOption2 ? `Couldn't find ${hubspotCliAccountNameOption2} in your hubspot.config.yaml` : `No default account found. Run 'hs auth' first.`
      );
      process.exit(1);
    }
    authedPortalId = account == null ? void 0 : account.accountId;
    const env2 = account == null ? void 0 : account.env;
    if (!env2) {
      throw new Error(
    ...
    authedEnv = env2;
    personalAccessKey = (account == null ? void 0 : account.authType) === PERSONAL_ACCESS_KEY_AUTH_METHOD.value ? account == null ? void 0 : account.personalAccessKey : null;
...
Local proxy functionality and module preview routes will not work without setting up authentication. You can work purely on modules locally at "/module/[module_name]", however the experience may be degraded due to missing account data.`
  ...
  const webRootProjectComponentFolder = getWebRootProjectComponentFolder(
    absoluteWebRoot,
    projectConfigPath
  );
  if (authedPortalId) {
  ...
  if (fieldGenEnabledOption2) {
    const normalizedAbsoluteWebRoot = path17.normalize(absoluteWebRoot);
    startFieldsTypeGenWatcher(viteServer, normalizedAbsoluteWebRoot);
  }
